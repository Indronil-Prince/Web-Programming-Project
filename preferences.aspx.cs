...
public partial class preferences : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if(id!=0)
        SqlDataSource1.SelectCommand = "SELECT p.prouct_id, p.store, p.type, p.Product_name, p.brand, p.price, p.available, p.image FROM orderTable o join product p on o.product_name=p.product_name where user_id="+id+"order by store ;";
