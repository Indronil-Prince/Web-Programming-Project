...
public partial class cricketstore : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (url != "http://localhost:2521/cricketstore.aspx")
        {
            string value = Request.QueryString["search"].ToString();
            SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Cricket' and ([product_name] like '%" + value + "%' or [type] like '%" + value + "%' or [category] like '%" + value + "%' or [brand] like '%" + value + "%') ORDER BY [category] DESC, [type], [brand], [product_name]";
        }
        else {
            SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Cricket' and category='Jersey' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
                    Category1.Text = "⚫ Jersey";
            SqlDataSource2.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Cricket' and category='Shoes' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
                    Category2.Text = "⚫ Shoe";
            SqlDataSource3.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Cricket' and category='Bat' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
                    Category3.Text = "⚫ Bat";
            SqlDataSource4.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Cricket' and category='Ball' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
                    Category4.Text = "⚫ Ball";
        } 
    }
    ...
    protected void Search(object sender, EventArgs e)
    {
        ...
        Response.Redirect("cricketstore.aspx?search=" + str);
    }
