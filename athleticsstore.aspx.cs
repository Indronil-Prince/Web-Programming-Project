...
public partial class athleticsstore : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if(url!= "http://localhost:2521/athleticsstore.aspx")
        { 
            string value = Request.QueryString["search"].ToString();
            SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Athletics' and ([product_name] like '%" + value + "%' or [type] like '%" + value + "%' or [category] like '%" + value + "%' or [brand] like '%" + value + "%') ORDER BY [category] DESC, [brand], [product_name]";
        }
        else { SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Athletics' ORDER BY [category] DESC, [brand], [product_name]"; }
    }
    ...
    protected void Search(object sender, EventArgs e)
    {
        ...
        Response.Redirect("athleticsstore.aspx?search=" + str);
    }
}