...
public partial class basketballstore : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (url != "http://localhost:2521/basketballstore.aspx")
        {
            string value = Request.QueryString["search"].ToString();
            SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Basketball' and ([product_name] like '%" + value + "%' or [type] like '%" + value + "%' or [category] like '%" + value + "%' or [brand] like '%" + value + "%') ORDER BY [category] DESC, [type], [brand], [product_name]";
        }
        else {
            SqlDataSource1.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Basketball' and category='Jersey' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
            Category1.Text = "⚫ Jersey";
            SqlDataSource2.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Basketball' and category='Shoes' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
            Category2.Text = "⚫ Shoe";
            SqlDataSource3.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Basketball' and category='Ball' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
            Category3.Text = "⚫ Ball";
            //SqlDataSource4.SelectCommand = "SELECT * FROM [product] WHERE [store] = 'Basketball' and category='Ball' ORDER BY [category] DESC, [type], [brand], [product_name] ;";
            //Category4.Text = "⚫ Ball";
        }
    }
    ...
    protected void Search(object sender, EventArgs e)
    {
        ...
        Response.Redirect("basketballstore.aspx?search=" + str);
    }
